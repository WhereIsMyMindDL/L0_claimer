...
from settings import decimal_places, RETRY_COUNT, delay_wallets

delay_transactions = [10, 15]                                       # минимальная и максимальная задержка между транзакциями
value_eth = ['20', '30']                                            # минимальное и максимальное кол-во ETH для свапов и ликвы, в ковычках ("90") можно указать процент
...
EIP1559_CHAINS = ['Ethereum', 'Zora', 'Optimism', 'Arbitrum one']
...
def intro(wallets):
    ...
    print()
    print(f'| {Fore.LIGHTGREEN_EX}L0_claimer{Fore.RESET} |'.center(100, '='))
    print('\n')
...
def sign_and_send_transaction(self, transaction, gas=None):
    max_priority_fee, max_fee_per_gas = 0, None

    if self.rpc in EIP1559_CHAINS:
        max_priority_fee = self.w3.eth.max_priority_fee
        max_fee_per_gas = max_priority_fee + int(self.w3.eth.get_block("latest")["baseFeePerGas"] * 1.05)
        # print(max_priority_fee, max_fee_per_gas)
    if max_priority_fee and max_fee_per_gas:
        transaction['maxPriorityFeePerGas'] = max_priority_fee
        transaction['maxFeePerGas'] = max_fee_per_gas * 1.2
    else:
        transaction['gasPrice'] = int(self.w3.eth.gas_price * 1.2)
    gas = int(self.w3.eth.estimate_gas(transaction) * random.uniform(1.1, 1.2))
